...
var populateCurrentCacheNames = function (precacheConfig, cacheNamePrefix, baseUrl,$log) {
    ...
    caches.keys().then(function(allCacheNames) {
      return Promise.all(
        Object.keys(CurrentCacheNamesToAbsoluteUrl).filter(function(cacheName) {
          ...
          $log.debug('Adding URL "%s" to cache named "%s"', urlWithCacheBusting, cacheName);
          return caches.open(cacheName).then(function(cache) {
            ...
            return fetch(request.clone()).then(function(response) {
              if (response.status == 200) {
                ...
                $log.error('Request for %s returned a response with status %d, so not attempting to cache it.',
        ...
        return Promise.all(
          allCacheNames.filter(function(cacheName) {
            ...
            $log.debug('Deleting out-of-date cache "%s"', cacheName);
            return caches.delete(cacheName);
          })
        )
      });
...
self.addEventListener('message', function(event) {
  if (event.data.command == 'delete_all') {
    $log.debug('About to delete all caches...');
    deleteAllCaches().then(function() {
      $log.debug('Caches deleted.');
      event.ports[0].postMessage({
        error: null
      });
    }).catch(function(error) {
      $log.debug('Caches not deleted:', error);
      event.ports[0].postMessage({
        error: error
      });
    });
          ...
          return cache.keys().then(function(keys) {
            return cache.match(keys[0]).then(function(response) {
              return response || fetch(event.request).catch(function(e) {
                $log.error('Fetch for "%s" failed: %O', urlWithoutIgnoredParameters, e);
              });
            });
          });
        }).catch(function(e) {
          $log.error('Couldn\'t serve response for "%s" from cache: %O', urlWithoutIgnoredParameters, e);
